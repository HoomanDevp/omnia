...
public class CustomEnvironmentPostProcessor implements EnvironmentPostProcessor, Ordered {
    ...
    private static final String MODULE_YAML_NAME = "oraxa.yml";
    ...
    public void postProcessEnvironment(ConfigurableEnvironment environment, SpringApplication application) {
        ...
        try {
            ...
            Resource[] oraxaResources = resolver.getResources(MODULE_YAML_PATTERN);
            for (Resource resource : oraxaResources) {
    ...
    private ModuleConfig parseModuleConfig(Resource resource, YamlPropertySourceLoader yamlLoader) throws IOException {
        ...
        for (PropertySource<?> propertySource : propertySources) {
            ...
            Integer order = (Integer) propertySource.getProperty("oraxa.module.order");
            if (order != null)
                config.order = order;
            ...
            config.ymlOrder = ((Map<String, Object>) propertySource.getSource())
                    .entrySet()
                    .stream()
                    .filter(q -> q.getKey().startsWith("oraxa.module.yml-order."))
                    .collect(Collectors.toMap(
                            q -> q.getKey().replace("oraxa.module.yml-order.", ""),
